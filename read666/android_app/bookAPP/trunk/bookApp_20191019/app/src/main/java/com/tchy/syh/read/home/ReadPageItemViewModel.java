...
public class ReadPageItemViewModel extends BaseViewModel {
    ...
    public ObservableList<ReadListItemVm> readList = new ObservableArrayList<>();
    public ItemBinding<ReadListItemVm> readItemsBinding = ItemBinding.of(BR.vm, R.layout.read_list_item);


    private LearnCate.DataBean bean;

    public ReadPageItemViewModel(Context context, LearnCate.DataBean bean) {
        super(context);
        this.bean = bean;
        onCreate();
    }
...
//        params.put("fid", bean.getId());
        ...
        Observable<ReadListResp> observable = RetrofitClient.getInstance().create(ApiService.class)
                .news_list(FormdataRequestUtil.getFormDataRequestParams(params))
        ...
        observable.subscribe(res -> {
            ...
            Observable.fromIterable(res.getData().getList()).subscribe(bean -> {
                readList.add(new ReadListItemVm(context, bean));
                    }
            );
