...
public class RoundImageView extends AppCompatImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (width >= minWidth && height > minHeight) {
            Path path = new Path();
            //四个角：右上，右下，左下，左上
            path.moveTo(leftTopRadius, 0); //不会进行绘制，只用于移动移动画笔。
            path.lineTo(width - rightTopRadius, 0);//用于进行直线绘制。
            path.quadTo(width, 0, width, rightTopRadius);//用于绘制圆滑曲线，即贝塞尔曲线。(x1,y1) 为控制点，(x2,y2)为结束点。
...
//            path.lineTo(width-20, height - rightBottomRadius-20);
//            path.quadTo((width-20)/2, height, 20, height - rightBottomRadius-20);
//
//            path.lineTo(20, 20);
