...
public class AudioPageItemVM extends BaseViewModel {
    ...
    public ObservableList<CollectAudioItemVM> list = new ObservableArrayList<>();
    public ItemBinding<CollectAudioItemVM> itemBindings = ItemBinding.of(BR.vm, R.layout.collect_audio_item);


    public LearnCate.DataBean bean;
    public AudioPageItemVM(Context context) {
        super(context);
        onCreate();
    ...
    public void getList(boolean isRefresh) {
        ...
        observable.subscribe(res -> {
            ...
            Observable.fromIterable(res.getData().getList()).subscribe(bean -> {
                        list.add(new CollectAudioItemVM(context, bean));
                    }
            );
            if (isRefresh)
