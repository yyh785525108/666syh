...
public class Custom1ImageView extends AppCompatImageView {
    ...
    public Custom1ImageView(Context context) {
        this(context, null);
        init(context, null);
    }

    public Custom1ImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        init(context, attrs);
    }

    public Custom1ImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (width >= minWidth && height > minHeight) {
            ...
            path.lineTo(width , 2);//用于进行直线绘制。
            path.quadTo((width)/2, leftTopRadius+2, 0, 2);//用于绘制圆滑曲线，即贝塞尔曲线。(x1,y1) 为控制点，(x2,y2)为结束点。
            path.lineTo(0, 0);
